...
运行时参数 p (外部代价版):
    [x_ref, y_ref, theta_ref, v_ref, omega_ref, a_ref, alpha_ref,
     d_esdf, x_esdf_lin, y_esdf_lin, grad_esdf_x, grad_esdf_y, weight_scale,
     q_pos, q_theta, q_vel,
     r_lin, r_ang,
...
说明:
    ...
    - ESDF 使用一阶线性化距离:
      d_lin = d_esdf + grad_esdf_x * (x - x_esdf_lin) + grad_esdf_y * (y - y_esdf_lin)
...
class WheellegModel:
    def __init__(self, enable_lag_model: bool = DEFAULT_ENABLE_LAG_MODEL):
        ...
        self.d_esdf = ca.SX.sym('d_esdf')                # 线性化点处 ESDF 距离 (m)
        self.x_esdf_lin = ca.SX.sym('x_esdf_lin')        # ESDF 线性化点 x (m)
        self.y_esdf_lin = ca.SX.sym('y_esdf_lin')        # ESDF 线性化点 y (m)
        self.grad_esdf_x = ca.SX.sym('grad_esdf_x')      # ESDF 距离对 x 的梯度
        self.grad_esdf_y = ca.SX.sym('grad_esdf_y')      # ESDF 距离对 y 的梯度
        ...
        self.params = ca.vertcat(
            ...
            self.d_esdf, self.x_esdf_lin, self.y_esdf_lin,
            self.grad_esdf_x, self.grad_esdf_y, self.weight_scale,
    ...
    def esdf_cost_expr(self, safe_dist):
        """ESDF 障碍物排斥违反量（一阶线性化距离）。"""
        d_lin = (
            self.d_esdf
            + self.grad_esdf_x * (self.x - self.x_esdf_lin)
            + self.grad_esdf_y * (self.y - self.y_esdf_lin)
        )
        violation = ca.fmax(0.0, safe_dist - d_lin)
