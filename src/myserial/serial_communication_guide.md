...
1. [USB CDC 通信原理](#1-usb-cdc-通信原理stm32-与-linux-pc-的完整链路)
   - [1.1 硬件层](#11-硬件层hardware-layer)
   - [1.2 USB CDC 协议栈](#12-usb-cdc-协议栈software-stack)
   - [1.3 虚拟串口的"虚拟"在哪里](#13-虚拟串口的虚拟在哪里)
   - [1.4 USB CDC 缓冲区架构与优化策略](#14-usb-cdc-缓冲区架构与优化策略) ⭐ 新增
   - [1.5 Linux 配置与连接流程](#15-linux-配置与连接流程)
...
### 1.4 USB CDC 缓冲区架构与优化策略

#### **USB Bulk 传输的包大小限制**

USB CDC 使用 **Bulk Transfer（批量传输）** 来传输数据，不同 USB 速度模式下的最大包大小：

| USB 模式 | 最大包大小 | 实际传输速度 | 典型应用 |
|---------|-----------|-------------|---------|
| **Low-Speed** | 8 字节 | 1.5 Mbps | 键盘、鼠标 |
| **Full-Speed** | **64 字节** ⭐ | 12 Mbps | **大多数 STM32** |
| **High-Speed** | **512 字节** | 480 Mbps | 高性能设备 |

**关键结论**：
- **STM32 常用的是 Full-Speed USB**，单次 Bulk 传输最大 **64 字节**
- 如果你的帧大小是 95 字节，需要 **至少 2 个 USB 包** 才能完整传输
- Linux 内核会自动处理分包/组包，应用层无感知

---

#### **多层缓冲区架构**

从 STM32 到 Linux 应用程序，数据经过 **5 层缓冲**：
...
└────────────────────────┬────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│ [层 2] USB Device Endpoint FIFO (硬件)                          │
│        size: 64B (Full-Speed) 或 512B (High-Speed)              │
│        作用: USB 控制器硬件发送队列                              │
└────────────────────────┬────────────────────────────────────────┘
                         │ USB 物理传输 (D+/D-)
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│ [层 3] Linux USB Host Controller Buffer                        │
│        size: 由 USB 主机控制器驱动管理 (通常几 KB)               │
│        作用: EHCI/XHCI 硬件接收缓冲                              │
└────────────────────────┬────────────────────────────────────────┘
                         │ urb_complete()
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│ [层 4] Kernel TTY Flip Buffer (内核空间)                        │
│        size: 默认 64KB (可调: /sys/class/tty/ttyACM0/bufsize)   │
│        作用: 内核 TTY 层的环形缓冲区                             │
└────────────────────────┬────────────────────────────────────────┘
                         │ read()
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│ [层 5] 应用层缓冲区 (用户空间)                                   │
│        size: 自定义 (本项目: 1 字节 vs 128 字节)                 │
│        作用: std::deque<uint8_t> 或 std::array<uint8_t, 256>    │
└─────────────────────────────────────────────────────────────────┘
```
...
1. **USB 包对齐原则**  
   - Full-Speed USB: 每次 Bulk 传输最多 64B
   - 建议读取大小：**64 的整数倍**（64B, 128B, 192B, 256B）
   - 原因：避免浪费 USB 带宽，减少系统调用次数

2. **帧大小考虑**  
   - 本项目帧大小：41B (发送) / 95B (接收)
   - 95B 需要 2 个 USB 包 (64 + 31)
   - **推荐 128B**：恰好容纳 1 个完整帧 + 部分下一帧
   - 激进配置 192B：能容纳 2 个完整帧

3. **系统调用开销**  
   ```
   单字节读取:   95 次 read() 系统调用/帧  →  高 CPU 开销
   128B 批量读:  约 1 次 read() 系统调用/帧  →  低开销
   ```

4. **延迟考虑**  
   - 批量读取不会增加延迟（内核有数据立即返回）
   - `async_read_some()` 是非阻塞的，有多少读多少

**配置建议**：

| 场景 | 推荐大小 | 理由 |
|------|---------|------|
| **低频通信** (< 10Hz) | 64B | 节省内存，单个 USB 包 |
| **中频通信** (10-50Hz) | **128B** ⭐ | 平衡性能和内存，2 个 USB 包 |
| **高频通信** (> 50Hz) | 192B 或 256B | 减少系统调用，4 个 USB 包 |
| **实时性要求极高** | 单字节 | 最低延迟（不推荐） |

**本项目配置**：
```yaml
# ROS2 参数配置
enable_batch_read: true
batch_read_size: 128  # 2×64B, 适合 95B 帧 + 50Hz 频率
```
...
**查看当前缓冲区大小**：
```bash
# 查看 TTY 缓冲区大小（默认 64KB）
cat /sys/class/tty/ttyACM0/bufsize
```

**调整缓冲区大小**（需要 root 权限）：
```bash
# 增大到 128KB（高频大数据量场景）
echo 131072 | sudo tee /sys/class/tty/ttyACM0/bufsize

# 减小到 16KB（低延迟场景，减少缓存）
echo 16384 | sudo tee /sys/class/tty/ttyACM0/bufsize
```

**何时需要调整**：
- **缓冲区溢出警告**：内核日志出现 `tty_insert_flip_string: buffer overflow`
- **高频大包通信**：例如 100Hz × 500B/帧 = 50KB/s
- **极低延迟要求**：减小缓冲区，强制应用层立即处理
...
```cpp
uint8_t read_byte_;
asio::async_read(port_, asio::buffer(&read_byte_, 1), callback);
```
- ✅ 优点：代码简单，最低延迟（理论上）
- ❌ 缺点：95 次系统调用/帧，CPU 开销巨大
- 📊 实测性能：~95ms 延迟（主要是系统调用开销）

**方案 2：批量读取（优化实现）**
```cpp
std::array<uint8_t, 256> read_buffer_;
port_.async_read_some(asio::buffer(read_buffer_, 128), callback);
```
- ✅ 优点：大幅减少系统调用（95 倍减少），低 CPU 开销
- ✅ 优点：符合 USB 传输特性（64B 对齐）
- ❌ 缺点：代码稍复杂（需要循环处理多个字节）
- 📊 预期性能：~1ms 延迟

**方案 3：固定大小读取（不推荐）**
```cpp
asio::async_read(port_, asio::buffer(buffer, FRAME_SIZE), callback);
```
- ❌ 缺点：如果数据未到齐会阻塞，丢失数据灵活性
- ❌ 缺点：无法处理多帧并发到达
...
```cpp
void parse_buffer() {
    const int MAX_BUFFER_SIZE = 1024;  // 根据帧大小设置
...
void parse_buffer() {
    if (buffer_.size() > MAX_BUFFER_SIZE) {
        // 只保留最后一个完整帧长度的数据
        size_t keep_size = WHOLE_GET_LEN * 2;
        if (buffer_.size() > keep_size) {
            buffer_.erase(buffer_.begin(), 
                         buffer_.end() - keep_size);
        }
        RCLCPP_WARN(this->get_logger(), 
            "缓冲区溢出，保留最后 %zu 字节", keep_size);
    }
}
```
...
**方法 1：系统调用计数**
```bash
# 使用 strace 统计 read() 调用次数
strace -c -e read ros2 run myserial serial_node 2>&1 | grep read
```

**方法 2：RTT 测量**（本项目已实现）
```cpp
// 在发送帧中嵌入时间戳，STM32 回传，计算往返延迟
float timestamp = fmod(now_ms, 10000.0);
send_frame._speed_w = timestamp;  // 利用空闲字段
// ... STM32 回传到 _target_position_x
double rtt = now - received_timestamp;
```

**方法 3：dmesg 监控内核消息**
```bash
# 监控 USB/TTY 相关错误
dmesg -w | grep -E "tty|usb|cdc"
```
