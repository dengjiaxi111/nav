...
double clampDot(double dot) {
    return std::max(-1.0, std::min(1.0, dot));
}

double angleBetweenUnsigned(const Eigen::Vector3d& a, const Eigen::Vector3d& b) {
    const double dot = std::abs(a.normalized().dot(b.normalized()));
    return std::acos(clampDot(dot));
}
...
class LidarRollCalibrator : public rclcpp::Node {
public:
    LidarRollCalibrator() : Node("lidar_roll_calibrator") {
        ...
        candidate_base_z_min_ = declare_parameter<double>("candidate_base_z_min", -1.2);
        candidate_base_z_max_ = declare_parameter<double>("candidate_base_z_max", 0.25);
        ...
        search_half_width_deg_ = declare_parameter<double>("search_half_width_deg", 20.0);
        search_step_deg_ = declare_parameter<double>("search_step_deg", 0.02);
        report_once_ = declare_parameter<bool>("report_once", true);

        odom_sub_ = create_subscription<nav_msgs::msg::Odometry>(
            odom_topic_, rclcpp::QoS(20),
            std::bind(&LidarRollCalibrator::odomCallback, this, std::placeholders::_1));
        ...
        if (pointcloud_type_ == "pointcloud2") {
            pc2_sub_ = create_subscription<sensor_msgs::msg::PointCloud2>(
                ...
                std::bind(&LidarRollCalibrator::pointCloud2Callback, this, std::placeholders::_1));
            ...
            livox_sub_ = create_subscription<livox_ros_driver2::msg::CustomMsg>(
                ...
                std::bind(&LidarRollCalibrator::livoxCallback, this, std::placeholders::_1));
            RCLCPP_INFO(get_logger(), "Subscribing Livox CustomMsg: %s", livox_topic_.c_str());
        }

        RCLCPP_INFO(
            get_logger(),
            "Current base_link->livox_frame rpy: roll=%.6f pitch=%.6f yaw=%.6f rad",
            current_roll_, current_pitch_, current_yaw_);
        RCLCPP_INFO(get_logger(), "Waiting for stationary odom and ground points...");
    }
...
private:
    ...
    bool odomStationary() const {
        ...
        const double linear =
            std::hypot(t.linear.x, t.linear.y, t.linear.z);
        const double angular =
            std::hypot(t.angular.x, t.angular.y, t.angular.z);
        return linear <= max_stationary_linear_speed_ &&
               angular <= max_stationary_angular_speed_;
    }

    Eigen::Vector3d currentUpInBase() const {
    ...
    void calibrate() {
        ...
        const Eigen::Vector3d expected_normal_lidar =
            (r_current.transpose() * up_base).normalized();
        ...
        seg.setAxis(Eigen::Vector3f(
            expected_normal_lidar.x(), expected_normal_lidar.y(), expected_normal_lidar.z()));
        ...
        if (normal_lidar.norm() < 1e-6) {
            RCLCPP_WARN(get_logger(), "Fitted plane normal is invalid; collecting again.");
            resetCollection();
            return;
        }
        normal_lidar.normalize();
        if (normal_lidar.dot(expected_normal_lidar) < 0.0) {
        ...
        double best_roll = current_roll_;
        double best_error = std::numeric_limits<double>::infinity();
        const double start = current_roll_ - degToRad(search_half_width_deg_);
        const double end = current_roll_ + degToRad(search_half_width_deg_);
        const double step = std::max(degToRad(search_step_deg_), 1e-5);
        for (double roll = start; roll <= end; roll += step) {
            const Eigen::Matrix3d r_base_lidar =
                rpyToMatrix(roll, current_pitch_, current_yaw_);
            const double error =
                angleBetweenUnsigned(r_base_lidar * normal_lidar, up_base);
            if (error < best_error) {
                best_error = error;
                best_roll = roll;
            }
        ...
        const double current_error =
            angleBetweenUnsigned(r_current * normal_lidar, up_base);

        RCLCPP_INFO(get_logger(), "========== LiDAR Roll Calibration Result ==========");
        RCLCPP_INFO(
        ...
        RCLCPP_INFO(
            get_logger(), "Ground normal in livox_frame: [%.6f, %.6f, %.6f]",
            normal_lidar.x(), normal_lidar.y(), normal_lidar.z());
        RCLCPP_INFO(
            get_logger(), "Up direction in base_link from odom: [%.6f, %.6f, %.6f]",
            up_base.x(), up_base.y(), up_base.z());
        RCLCPP_INFO(
            get_logger(), "Current roll: %.6f rad (%.3f deg)",
            current_roll_, radToDeg(current_roll_));
        RCLCPP_INFO(
            get_logger(), "Recommended roll: %.6f rad (%.3f deg)",
            best_roll, radToDeg(best_roll));
        RCLCPP_INFO(
            get_logger(), "Delta roll: %.6f rad (%.3f deg)",
            best_roll - current_roll_, radToDeg(best_roll - current_roll_));
        RCLCPP_INFO(
            get_logger(), "Residual tilt: current=%.3f deg, recommended=%.3f deg",
            radToDeg(current_error), radToDeg(best_error));
        RCLCPP_INFO(
            get_logger(),
            "Update static_transform_publisher --roll to %.6f, keep pitch=%.6f yaw=%.6f",
            best_roll, current_pitch_, current_yaw_);
    ...
    double candidate_base_z_min_{-1.2};
    double candidate_base_z_max_{0.25};
    double candidate_xy_abs_max_{8.0};
    double max_stationary_linear_speed_{0.08};
    double max_stationary_angular_speed_{0.08};

    double search_half_width_deg_{20.0};
    double search_step_deg_{0.02};
    bool report_once_{true};
...
int main(int argc, char** argv) {
    ...
    rclcpp::spin(std::make_shared<LidarRollCalibrator>());
    rclcpp::shutdown();
    return 0;
}
