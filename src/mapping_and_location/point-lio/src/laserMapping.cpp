...
geometry_msgs::msg::TransformStamped extrin_tf;
  ...
  const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr & pubLaserCloudFullRes)
{
  if (scan_pub_en) {
    ...
    // slh: 将世界系点云从初始imu位置转到初始云台中心位置
    int size = feats_down_world->points.size();
    PointCloudXYZI::Ptr cloud_to_transform(new PointCloudXYZI(size,1));
    pcl::copyPointCloud(*feats_down_world,*cloud_to_transform);

    pcl::transformPointCloud(*cloud_to_transform,*cloud_to_transform,extrin_trans);
    pcl::toROSMsg(*cloud_to_transform, laserCloudmsg);
  ...
  const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr & pubLaserCloudFull_body)
{
  ...
  laserCloudmsg.header.stamp = get_ros_time(lidar_end_time);
  laserCloudmsg.header.frame_id = "livox_left";
  ...
  std::shared_ptr<tf2_ros::TransformBroadcaster> & tf_br)
{
  ...
  if (tf_send_en) {
    ...
    transform.header.stamp = rclcpp::Time(odomAftMapped.header.stamp);

    // 下面进行坐标变换
    tf2::Transform lidar2gimbal;
    tf2::Transform final_trans;
    tf2::Transform odom_trans;
    tf2::fromMsg(extrin_tf.transform, lidar2gimbal);
    tf2::fromMsg(transform.transform, odom_trans);
    final_trans = lidar2gimbal * odom_trans * lidar2gimbal.inverse();
    transform.transform.translation.x = final_trans.getOrigin().getX();
    transform.transform.translation.y = final_trans.getOrigin().getY();
    transform.transform.translation.z = final_trans.getOrigin().getZ();
    transform.transform.rotation.w = final_trans.getRotation().getW();
    transform.transform.rotation.x = final_trans.getRotation().getX();
    transform.transform.rotation.y = final_trans.getRotation().getY();
    transform.transform.rotation.z = final_trans.getRotation().getZ();

    // 顺便发布一个yaw角不变的“伪车头”坐标系
    geometry_msgs::msg::TransformStamped transform_static;
    transform_static.header.stamp = rclcpp::Time(odomAftMapped.header.stamp);
    ...
    tf2::Quaternion q;
    q.setRPY(0,0,-tf2::getYaw(transform.transform.rotation));
    transform_static.transform.rotation = tf2::toMsg(q);

    tf_br->sendTransform(transform);
    tf_br->sendTransform(transform_static);
  }
}
...
int main(int argc, char ** argv)
{
  ...
  {
    if(failed_cnt > 20){
		extrin_trans = Eigen::Affine3f::Identity();
		break;
    }
    try
    {
        // 查询 TF 变换：从 "livox_left" 到 "base_link"
        extrin_tf = tf_buffer->lookupTransform("base_link", "livox_left", tf2::TimePointZero);
        Eigen::Affine3f ex_trans = Eigen::Affine3f::Identity();
        Eigen::Quaternionf q(extrin_tf.transform.rotation.w, extrin_tf.transform.rotation.x, extrin_tf.transform.rotation.y, extrin_tf.transform.rotation.z);
       ex_trans.rotate(q);
        ex_trans.pretranslate(Eigen::Vector3f(extrin_tf.transform.translation.x, extrin_tf.transform.translation.y, extrin_tf.transform.translation.z ));
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      ...
      if (feats_down_size > 4) {
        ...
        // slh: 限制问题规模
        // static size_t max_voxel_points = 1000000;
        // if (ivox_->Size() > max_voxel_points) {
        //     ivox_->Clear(); // 或 ivox_.reset(new IVoxType(ivox_options_))
        //     RCLCPP_WARN(LOGGER, "ivox size exceeded, reset to avoid CPU overload.");
        // }
      }
      t5 = omp_get_wtime();
      /******* Publish points *******/
