  ...
  deque<double> & time_lidar, const int required_frame_num, int scan_count)
{
  ...
  double extrin_x = extrin_trans.translation().x();
  double extrin_y = extrin_trans.translation().y();
  double extrin_z = extrin_trans.translation().z();
  for (uint i = 1; i < plsize; i++) {
    if (
      ...
      if (valid_point_num % point_filter_num == 0) {
        ...
        // slh: 修改为计算点云到车体中心的距离
        double dist =
          (pl_full[i].x - extrin_x) * (pl_full[i].x - extrin_x) + (pl_full[i].y - extrin_y) * (pl_full[i].y - extrin_y) + (pl_full[i].z - extrin_z) * (pl_full[i].z - extrin_z);
