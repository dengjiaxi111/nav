...
public:
    LocalizationInitializer() : Node("localization_initializer") {
        ...
        declare_parameter("use_initialpose_as_static_tf", false);

        // ==================== 读取参数 ====================
        map_file_ = get_parameter("map_file").as_string();
        auto_initialize_ = get_parameter("auto_initialize").as_bool();
        use_initialpose_as_static_tf_ = get_parameter("use_initialpose_as_static_tf").as_bool();
        ...
        // ==================== 加载地图 ====================
        if (map_file_.empty()) {
            startTfFallbackWithoutMap("参数 'map_file' 未设置");
            return;
        }
        
        if (!loadMapCloud()) {
            startTfFallbackWithoutMap("地图加载失败");
            return;
        }
        ...
        if (use_initialpose_as_static_tf_) {
            RCLCPP_INFO(get_logger(), "静态 TF 模式: 请在 RViz 中使用 '2D Pose Estimate' 设置 map->odom");
...
private:
    ...
    void startTfFallbackWithoutMap(const std::string& reason) {
        no_pcd_map_tf_fallback_ = true;
        localization_initialized_ = true;

        publishInitialMapFrame();
        publishStatus("未加载 PCD 地图，已发布初始 map → odom = identity，等待 RViz 初始位姿");

        RCLCPP_WARN(
            get_logger(),
            "%s，进入无 PCD 地图 TF 回退模式: 已发布 map->odom 初始值 [0, 0, 0]，可用 RViz /initialpose 更新 base_link 在 map 中的位置",
            reason.c_str());
    }
    ...
    void initialPoseCallback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg) {
        ...
        if (no_pcd_map_tf_fallback_) {
            publishMapToOdomTF(user_initial_guess_);
            localization_initialized_ = true;
            publishStatus("无 PCD 地图模式：已根据 RViz 初始位姿更新 map → odom TF");
            return;
        }

        if (use_initialpose_as_static_tf_) {
            publishStaticMapToOdomTF(user_initial_guess_);
            localization_initialized_ = true;
            publishStatus("已使用 RViz 初始位姿发布静态 map → odom TF");
            return;
        }
    ...
    void publishStaticMapToOdomTF(const Eigen::Matrix4f& T_map_to_odom) {
        geometry_msgs::msg::TransformStamped static_tf;
        static_tf.header.frame_id = "map";
        static_tf.child_frame_id = "odom";
        static_tf.header.stamp = now();

        static_tf.transform.translation.x = T_map_to_odom(0, 3);
        static_tf.transform.translation.y = T_map_to_odom(1, 3);
        static_tf.transform.translation.z = T_map_to_odom(2, 3);
        ...
        has_map_to_odom_tf_ = false;
        static_tf_broadcaster_->sendTransform(static_tf);

        RCLCPP_INFO(get_logger(), "🔗 map → odom 静态 TF 已由 RViz 初始位姿发布");
    }
    ...
    bool use_initialpose_as_static_tf_ = false;
    bool no_pcd_map_tf_fallback_ = false;
