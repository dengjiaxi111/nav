...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if mode == "map":
            ...
            # 创建消息接口 - 唯一节点名
            msg_interface = MessageInterface(node_name='referee_simulator_map')
            
            # 创建哨兵控制器 - 唯一节点名
            sentry_controller = SentryController(game_state, node_name='sentry_controller_map')
            ...
            try:
                # 控制面板使用唯一的节点名
                msg_interface = MessageInterface(node_name='referee_simulator_control')
                print("✓ 已创建消息接口（节点名：referee_simulator_control）")
            except Exception as e:
