...
# 机器人编号
ROBOT_IDS = [1, 2, 3, 4, 7]
RED_SENTRY_ROBOT_ID = 7
BLUE_SENTRY_ROBOT_ID = 107

def is_blue_robot_id(robot_id):
    return int(robot_id) == BLUE_SENTRY_ROBOT_ID

def get_our_team(robot_id):
    return 'blue' if is_blue_robot_id(robot_id) else 'red'

def get_enemy_team(robot_id):
    return 'red' if is_blue_robot_id(robot_id) else 'blue'
