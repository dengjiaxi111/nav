...
namespace small_point_lio {
    ...
    public:
        ...
        using measurement_model_point_batch = std::function<void(const state &, batch_point_measurement_result &)>;
    ...
    public:
        ...
        inline void init_batch(const measurement_model_point_batch &h_point_batch) {
            this->h_point_batch = h_point_batch;
        }
        ...
        inline void predict_cov(double timestamp, Eigen::Matrix<state::value_type, state::DIM, state::DIM> &Q) {
            ...
            if (dt_cov > 0) [[likely]] {
                ...
                F.block<3, 3>(state::velocity_index, state::rotation_index) = -x.rotation * hat<state::value_type>(x.acceleration) * dt_cov;
