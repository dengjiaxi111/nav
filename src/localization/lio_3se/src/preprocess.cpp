...
void Preprocess::process_multi(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg1,const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg2, PointCloudXYZI::Ptr& pcl_out)
{
  avia_handler_multi(msg1,msg2);
  *pcl_out = pl_surf;
}
...
void Preprocess::avia_handler_multi(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg1, const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg2)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  double t1 = omp_get_wtime();
  int plsize = msg1->point_num + msg2->point_num;
  // cout<<"plsie: "<<plsize<<endl;
  ...
  uint valid_num = 0;
  for (uint i = 1; i < msg1->point_num; i++)
  {
    if ((msg1->points[i].line < N_SCANS) &&
        ((msg1->points[i].tag & 0x30) == 0x10 || (msg1->points[i].tag & 0x30) == 0x00))
    {
      valid_num++;
      if (valid_num % point_filter_num == 0)
      {
        pl_full[i].x = msg1->points[i].x;
        pl_full[i].y = msg1->points[i].y;
        pl_full[i].z = msg1->points[i].z;
        pl_full[i].intensity = msg1->points[i].reflectivity;
        pl_full[i].curvature = msg1->points[i].offset_time /
                                float(1000000);  // use curvature as time of each laser points, curvature unit: ms
        if (((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7)
            || (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7)
            || (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7))
            && (pl_full[i].x * pl_full[i].x + (pl_full[i].y -0.12 ) * (pl_full[i].y-0.12) + (pl_full[i].z+0.09) * (pl_full[i].z+0.09) > (blind * blind)))
        {
          pl_surf.push_back(pl_full[i]);
        }
      }
  ...
  for (uint i = 1; i < msg2->point_num; i++)
  {
    if ((msg2->points[i].line < N_SCANS) &&
        ((msg2->points[i].tag & 0x30) == 0x10 || (msg2->points[i].tag & 0x30) == 0x00))
    {
      valid_num++;
      if (valid_num % point_filter_num == 0)
      {
        pl_full[i].x = msg2->points[i].x;
        pl_full[i].y = msg2->points[i].y;
        pl_full[i].z = msg2->points[i].z;
        pl_full[i].intensity = msg2->points[i].reflectivity;
        pl_full[i].curvature = msg2->points[i].offset_time /
                                float(1000000);  // use curvature as time of each laser points, curvature unit: ms
        if (((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7)
            || (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7)
            || (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7))
