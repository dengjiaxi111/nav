# src/control_panel.py - 省赛版本（机器人ID输入框放在哨兵下方）
...
class ControlPanel:
    def __init__(self, game_state, msg_interface):
        ...
        # 控制面板尺寸
        self.width = CONTROL_WIDTH
        self.height = CONTROL_HEIGHT
        
        # 创建窗口
        self.screen = pygame.display.set_mode((self.width, self.height))
        pygame.display.set_caption("RoboMaster Control Panel - 省赛版")
    ...
    def init_ui_elements(self):
        ...
        # 红方机器人信息（只有1,3,7）
        y_offset = red_y
        robot_labels = ["Hero", "Infantry3", "Sentry"]
        
        for i, robot_id in enumerate([1, 3, 7]):
            ...
            if robot:
                ...
                # 发弹量输入框（1和3有发弹量，7也有，但省赛可能无）
                if robot_id in [1, 3, 7]:
        ...
        # ========== 机器人ID输入框（放在哨兵下方） ==========
        # 在哨兵（robot_id=7）之后添加ID输入框，紧接其下方
        self.add_display_box('robot_id_label', {
            'rect': pygame.Rect(red_x, y_offset, 240, 25),
            'text': "Robot ID (0=Red, 1=Blue):",
            'color': WHITE,
            'font': self.normal_font
        })
        ...
        self.add_input_box('our_robot_id', {
            'rect': pygame.Rect(red_x + 250, y_offset, 100, 35),
            'label': "ID",
            'active': False,
            'type': 'int',
            'min': 0,
    ...
    def update_robot_id(self, value):
        ...
        self.save_and_sync_state()
        print(f"Updated robot ID to {value} (0=Red, 1=Blue)")
    ...
    def draw_ui(self):
        ...
        title = self.title_font.render("RoboMaster Control Panel - 省赛版", True, YELLOW)
        ...
        # 分隔线
        mid_x = self.width // 2
        pygame.draw.line(self.screen, WHITE, (mid_x, 90), (mid_x, self.height - 50), 1)
