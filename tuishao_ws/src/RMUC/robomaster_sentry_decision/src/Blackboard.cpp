...
bool Blackboard::loadConfigFromYAML(const std::string& filepath) {
    try {
        ...
        config_.init_pre_attack.x = config["init_pre_attack_x"].as<double>();
        config_.init_pre_attack.y = config["init_pre_attack_y"].as<double>();
        ...
        RCLCPP_ERROR(rclcpp::get_logger("sentry_decision.blackboard"),
                     "YAML loading failed: %s", e.what());
        return false;
    } catch (const std::exception& e) {
        RCLCPP_ERROR(rclcpp::get_logger("sentry_decision.blackboard"),
                     "Missing config: %s", e.what());
        return false;
    }
}
